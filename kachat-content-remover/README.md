# kachat-content-remover

A utility for removing all content created by a specific user from the K-indexer database.

## Overview

kachat-content-remover is a standalone tool that removes all records associated with a given public key from the K-indexer database. This tool is useful for data cleanup, user account deletion, or removing spam/malicious content.
...
- **Safe Execution**: Can run while kachat-transaction-processor and kachat-webserver are active

## What Gets Removed

kachat-content-remover deletes records from the following tables where `sender_pubkey` matches the target user:
...
```bash
cd kachat-content-remover && cargo run -- \
...
```bash
cargo build --release
./target/release/kachat-content-remover \
  --target-user 1234567890abcdef \
  --db-host localhost \
  --db-port <DB_PORT> \
  --db-name <DB_NAME> \
...
- kachat-transaction-processor schema must be initialized (tables must exist)
...
- **Concurrent Execution**: Safe to run while kachat-transaction-processor is active (uses proper transaction isolation)
...
```
Starting kachat-content-remover v0.1.0
...
## Comparison with kachat-database-cleaner

| Feature | kachat-content-remover | kachat-database-cleaner |
|---------|------------------|-------------------|
