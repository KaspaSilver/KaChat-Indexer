...
kachat-database-cleaner runs periodic purge operations to remove:
...
kachat-database-cleaner does **not** remove anything from:
- `k_broadcasts`
...
```bash
cd kachat-database-cleaner && cargo run -- \
...
```bash
cargo build --release
./target/release/kachat-database-cleaner \
  --user 1234567890abcdef \
  --data-retention 72 \
  --purge-interval 1800 \
  --db-host localhost \
...
- kachat-transaction-processor running (for database schema)
