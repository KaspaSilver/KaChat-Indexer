// kachat-admin — ops/admin dashboard for the KaChat indexer.
//
...
#[derive(Parser, Debug)]
#[command(name = "kachat-admin", about = "Ops/admin dashboard for the KaChat indexer")]
struct Args {
...
async fn main() -> anyhow::Result<()> {
    ...
    info!("kachat-admin connecting to database {}:{}/{}", args.db_host, args.db_port, args.db_name);
    ...
    // this covers kachat-admin starting first on a fresh DB).
    let _ = sqlx::query(
...
struct StatsResponse {
    ...
    ingest_last_60m: i64,
    db_size_bytes: i64,
    // On-disk size of the chat indexer's fjall store (the DMs/handshakes/payments/groups) — a
...
/// cheap even for a large store. Best-effort: unreadable entries are skipped; 0 if the dir is gone.
...
async fn get_stats(State(state): State<AppState>) -> Result<Json<StatsResponse>, ApiError> {
    ...
    let chat_store_bytes = tokio::task::spawn_blocking(move || dir_size(&chat_dir) as i64)
        .await
        .unwrap_or(0);
...
async fn post_remove(
    ...
    let (mentions, contents, votes, broadcasts, blocks, follows) = if req.dry_run {
        ...
        // from k_contents via FK). Mirrors kachat-content-remover's execute_removal.
        let row = sqlx::query(
            r#"
        ...
        .await
        .map_err(ApiError::db)?;
        info!("kachat-admin executed content removal for {}", pubkey);
        (
            row.get::<i64, _>("mentions"),
            row.get::<i64, _>("contents"),
            row.get::<i64, _>("votes"),
...
async fn post_denylist_add(
    ...
    let purged = row.get::<i64, _>("mentions")
        ...
        + row.get::<i64, _>("blocks")
        + row.get::<i64, _>("follows");
    info!("kachat-admin {} author {} and purged {} existing rows", kind, pk, purged);
...
/// Run `supervisorctl <action> <program>` inside this container (kachat-admin and the chat indexer
...
async fn post_settings(
    ...
    if let Some(b) = req.chat_indexer {
        ...
        info!("kachat-admin {} chat indexer via supervisorctl", action);
    }
...
async fn post_broadcast_delete(
    ...
    info!("kachat-admin deleted {} broadcast row(s)", deleted);
    Ok(Json(DeleteResult { deleted }))
}
...
async fn post_content_delete(
    ...
    let deleted = sqlx::query("DELETE FROM k_contents WHERE transaction_id = $1")
        ...
        .map_err(ApiError::db)?
        .rows_affected();
    info!("kachat-admin deleted {} content row(s) for tx {}", deleted, tx);
    Ok(Json(DeleteResult { deleted }))
}
